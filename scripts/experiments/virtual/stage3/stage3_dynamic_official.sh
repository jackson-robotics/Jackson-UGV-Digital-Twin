...
# Jackson Stage 3 C1 — official dynamic-cylinder runs R01–R999 (inflation 0.25, cost scaling 10.0)
# Usage per run:
...
#   5) stage3_dynamic_official.sh run R06
...
Ejecuta, por ejemplo:
  stage3_dynamic_official.sh run R06
TXT
}

valid_run() {
  [[ ${1:-} =~ ^R([0-9]{2,3})$ ]] ||
    red 'Usa un identificador entre R01 y R999'

  local run_number=$((10#${BASH_REMATCH[1]}))

  (( run_number >= 1 && run_number <= 999 )) ||
    red 'Usa un identificador entre R01 y R999'
}
...
cmd_run(){
  [[ $# -eq 1 ]] || red 'Uso: stage3_dynamic_official.sh run R06'
