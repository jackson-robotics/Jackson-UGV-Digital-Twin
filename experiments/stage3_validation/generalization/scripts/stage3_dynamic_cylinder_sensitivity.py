...
CONDITION_FILE = (
    ...
    "revision_r1_5_generalization/ACTIVE_CONDITION.txt"
)

with open(CONDITION_FILE, "r", encoding="utf-8") as f:
    ACTIVE_CONDITION = f.read().strip()

CYLINDER_Y_BY_CONDITION = {
    "G01_CYL_PLUS_Y":  0.1136,
    "G02_CYL_MINUS_Y": -0.1864,
    "G03_LOW_FRICTION": -0.0364,
    "G04_LIDAR_DEGRADED": -0.0364,
}

if ACTIVE_CONDITION not in CYLINDER_Y_BY_CONDITION:
    raise RuntimeError(
        f"Unsupported generalization condition: {ACTIVE_CONDITION}"
    )

CYLINDER_FINAL_Y = CYLINDER_Y_BY_CONDITION[ACTIVE_CONDITION]

CYLINDER_PARK_X = 0.6091
CYLINDER_PARK_Y = CYLINDER_FINAL_Y
