...
# Jackson Stage 3 — Reviewer 1.1 sensitivity campaign
# 15 independent virtual runs per condition.
#
# Natural workflow:
#   1) Select condition:  condition C00_BASE
...
DWB_CFG=/home/carlos/jackson_stage3_virtual/config/nav2_params_stage3_dwb_infl025.yaml
EKF_CFG=/home/carlos/jackson_dt_ws/config/ekf_stage3_virtual.yaml
DYNAMIC_SCRIPT=/home/carlos/jackson_dt_ws/revision_r1_1_sensitivity/scripts/stage3_dynamic_cylinder_sensitivity.py
CAMPAIGN_ROOT=/home/carlos/jackson_dt_ws/revision_r1_1_sensitivity
RUNS_ROOT=$CAMPAIGN_ROOT/runs
CONDITION_FILE=$CAMPAIGN_ROOT/ACTIVE_CONDITION.txt
STATE=/tmp/jackson_stage3_sensitivity
BAG_BASE=
PIDS=$STATE/pids
LOGS=$STATE/logs
...
load_condition(){
  [[ -f "$CONDITION_FILE" ]] || red "No hay condición seleccionada. Ejecuta: $0 condition C00_BASE"
  ...
  case "$CONDITION" in
    ...
    C03_B_MINUS5)
      WHEEL_RADIUS=0.03300
      WHEEL_SEPARATION=0.18810
      PHYSX_DAMPING=10000
      ;;
    C04_B_PLUS5)
      WHEEL_RADIUS=0.03300
      WHEEL_SEPARATION=0.20790
      PHYSX_DAMPING=10000
      ;;
    C05_D_MINUS10)
      WHEEL_RADIUS=0.03300
...
cmd_condition(){
  [[ $# -eq 1 ]] || red "Uso: $0 condition C00_BASE"

  case "$1" in
    C00_BASE|C01_R_MINUS5|C02_R_PLUS5|C03_B_MINUS5|C04_B_PLUS5|C05_D_MINUS10|C06_D_PLUS10)
      ;;
    *)
...
Ahora, en Isaac Sim:
  ...
  3. Rearma en Script Editor:
     exec(open("/home/carlos/jackson_dt_ws/revision_r1_1_sensitivity/scripts/stage3_dynamic_cylinder_sensitivity.py", encoding="utf-8").read())
